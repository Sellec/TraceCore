...
namespace System
{
    ...
    public static class GenericCollectionsExtension
    {
        ...
        {
            TValue value;
            return dictionary.TryGetValue(key, out value) ? value : (defaultValueProvider != null ? defaultValueProvider(key) : default(TValue));
        }
        ...
        {
            TValue value;
            return dictionary.TryGetValue(key, out value) ? value : (defaultValueProvider != null ? defaultValueProvider(key, createNewInfo) : default(TValue));
        }
        ...
        /// Возвращает значение из словаря <paramref name="dictionary"/> по ключу <paramref name="key"/>. Если такой ключ отсутствует, то вернет значение, полученное вызовом <paramref name="defaultValueProvider"/>.
        /// </summary>
        public static TValue GetValueOrDefault<TKey, TValue>(this IReadOnlyDictionary<TKey, TValue> dictionary, TKey key, Func<TKey, TValue> defaultValueProvider)
        {
            TValue value;
            return dictionary.TryGetValue(key, out value) ? value : (defaultValueProvider != null ? defaultValueProvider(key) : default(TValue));
        }
