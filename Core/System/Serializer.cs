...
namespace Conversive.PHPSerializationLibrary
{
    ...
    class Serializer
    {
        ...
        private bool XMLSafe = true; //This member tells the serializer wether or not to strip carriage returns from strings when serializing and adding them back in when deserializing
        ...
        private StringBuilder serialize(object obj, StringBuilder sb)
        {
            ...
            {
                var interfaceEnumerable = TraceCore.Types.TypeHelpers.ExtractGenericInterface(obj.GetType(), typeof(IEnumerable<>));
                if (interfaceEnumerable != null)
                {
                    ...
                    return this.serialize(arr, sb);
                }
                else
                {
                    var props = TraceCore.Utils.TypeHelper.ObjectToDictionary(obj);
                    var ht = new Hashtable((IDictionary)props);

                    return this.serialize(ht, sb);
                }
            }
        }
