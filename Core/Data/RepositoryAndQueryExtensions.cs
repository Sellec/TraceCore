    ...
    public static class RepositoryAndQueryExtensions
    {
        ...
        private static IQuery<T> GetRepositoryQuery<T>(IQueryable<T> source) where T : class
        {
            ...
            if (d == null)
            {
                ...
                throw new System.Data.EvaluateException("Данный запрос, к которому Вы пытаетесь применить метод расширения, не относится к механизму репозиториев TraceCore.");
            }

            return d;
        }

        #region Счетчик запросов
