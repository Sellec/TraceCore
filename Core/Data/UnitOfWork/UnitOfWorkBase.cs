...
namespace TraceCore.Data
{
    ...
    public abstract partial class UnitOfWorkBase : IDisposable
    {
        ...
        {
            var entityTypes2 = entityTypes.ToList().Merge(GetPropertiesTypes());
            ...
            _context = DataAccessProvider.Instance.DefaultProvider.CreateDataContext(entityTypes2.ToArray());
            GenerateProperties();
        }
        ...
        public IRepository<TEntity> Get<TEntity>() where TEntity : class
        {
            ...
            var repo = _repositoryCache.GetOrAdd(
                type,
                (t) => DataAccessProvider.Instance.DefaultProvider.CreateRepository<TEntity>(_context)
            );
